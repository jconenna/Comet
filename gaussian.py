...
def gaussian(x, width=1., center=0., height=None, param=None):
  ...
  '''
  if param != None:                     # unpack parameters, if necessary
    pdim = len(param)
    if pdim / 2 == pdim / 2.:
      pdim = pdim / 2
    else:
...
def gaussianguess(y, x=None):
  '''
    ...
    >>> print(g.gaussianguess(y, x))
    (array([ 3.,  3.]), (53, 53), 0.017683882565766154)
...
def fitgaussian(y, x=None, guess=None, lsargs={"maxfev":2000}, minout=None):
  ...
  # "ravel" the guess
  gss = np.append(guess[0], guess[1])
  gss = np.append(gss,      guess[2])
  ...
  if   success < 1 \
    ...
    raise ValueError, mesg + " No convergence.  Guess = "+str(guess) \
      +"; try a different guess, maxfev, or tolerances; see scipy.optimize.leastsq() docs."
